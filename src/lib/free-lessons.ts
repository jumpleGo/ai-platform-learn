...
const LESSONS: Record<number, FreeLessonContent> = {
  ...
  2: {
    ...
    bridgeTitle: 'Хороший промпт помогает один раз. Система контекста помогает постоянно',
    bridgeText: 'Если каждый раз заново объяснять проект в чате, качество будет зависеть от памяти человека и длины переписки. В полном workflow контекст, правила и навыки хранятся рядом с проектом и работают повторяемо.',
    ctaLabel: 'Посмотреть, как устроен полный AI-workflow',
