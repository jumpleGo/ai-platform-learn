...
// Границы можно задать явно (min/max) — для маленьких значений вилка от базы слишком широкая.
// На сервере возвращает null (рендерим плейсхолдер) — чтобы не было рассинхрона гидрации.
export function useLiveCount(base: number, min?: number, max?: number): number | null {
  const [n, setN] = useState<number | null>(null);
  const value = useRef(base);

  useEffect(() => {
    const lo = min ?? Math.max(1, Math.floor(base * 0.7));
    const hi = max ?? Math.ceil(base * 1.35) + 1;
    ...
    const tick = () => {
      // чаще ±1 (один пришёл/ушёл), иногда скачок чуть крупнее — но не шире самой вилки
      const step = Math.random() < 0.8 || hi - lo < 4 ? 1 : 2;
