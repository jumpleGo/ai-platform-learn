...
// Диалог теста отдельно от баннера-витрины: его же открывает кнопка
// на сцене-джелатории, у неё нет баннера с картинкой гуся.
export function QuizDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[calc(100%-1.5rem)] gap-0 p-0 sm:max-w-lg">
        <QuizBody onDone={() => onOpenChange(false)} />
      </DialogContent>
    </Dialog>
  );
}
    ...
    return (
      <div className="space-y-5 p-5 sm:p-7">
        <div className="space-y-2">
          <DialogTitle className="font-heading text-xl leading-tight font-bold text-balance sm:text-2xl">
            {result.verdict}
          </DialogTitle>
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-border bg-secondary/60 p-4">
          ...
          <p className="text-[15px] leading-snug font-bold">{result.title}</p>
        </div>
      ...
      <ul className="space-y-2">
        {question.options.map((option, i) => (
          <li key={option.label}>
            ...
            >
              <p className="text-[15px] leading-snug font-bold transition-colors group-hover:text-primary">
                {option.label}
              </p>
